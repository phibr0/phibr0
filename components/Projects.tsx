...
import { GitHubLogoIcon, PilcrowIcon, PlayIcon } from "@radix-ui/react-icons"
    ...
    links: [
      {
        ...
        icon: <PilcrowIcon width={20} height={20} />,
      },
      {
        name: "YouTube",
...
export const Projects = () => {
  ...
  return (
    <div>
    <Ticker duration={60} isPlaying={playing} onMouseEnter={() => setPlaying(false)} onMouseLeave={() => setPlaying(true)}>
      {ITEMS.map((item) => (
        <div
          className="h-40 py-4 text-left px-24"
          key={item.title}
        >
